...
--
-- Every assertion runs as the `authenticated` role impersonating one token
-- subject, so the policies are exercised rather than bypassed.
...
values (test.family_id('Family A'), 'Parent A', 'primary_guardian', true),
       (test.family_id('Family C'), 'Parent C', 'primary_guardian', true);

-- Each parent signs in; the token subject is attached to their contact.
select chat.link_contact_account(
  (select id from chat.contact where name = 'Parent A'), 'family:parent-a');
select chat.link_contact_account(
  (select id from chat.contact where name = 'Parent C'), 'family:parent-c');
...
-- Staff visibility ----------------------------------------------------------
select test.eq(test.count_as(test.subject_of('Owner A'), 'select count(*) from chat.family'),
               2, 'an admin can open any family (brief SS5)');

select test.eq(test.count_as(test.subject_of('Manager'), 'select count(*) from chat.family'),
               2, 'the manager sees every family');

-- Brief SS2: department staff see only families they hold a task on.
select test.eq(test.count_as(test.subject_of('Finance'), 'select count(*) from chat.family'),
...
select test.eq(test.count_as(test.subject_of('Finance'), 'select count(*) from chat.task'),
               1, 'and exactly their own task');

select test.eq(test.count_as(test.subject_of('Owner B'),
                             'select count(*) from chat.task'),
               1, 'admins see tasks on families they can open');

-- The family side reaches no base table ------------------------------------
select test.eq(test.count_as('family:parent-a',
...
select test.eq(test.count_as('family:parent-c',
                             'select count(*) from chat.my_messages'),
...
select test.eq(test.count_as(test.subject_of('Manager'), 'select count(*) from chat.audit_log'),
               1, 'the manager can read the audit log');

select test.eq(test.count_as(test.subject_of('Owner A'), 'select count(*) from chat.audit_log'),
...
select test.allowed_for(test.subject_of('Owner A'),
...
select test.denied_for(test.subject_of('Coverage A'),
  format($q$insert into chat.message (thread_id, author_type, author_id, on_behalf_mode,
                                      body, visibility)
...
select test.denied_for(test.subject_of('Finance'),
  format($q$insert into chat.message (thread_id, author_type, author_id, on_behalf_mode,
                                      body, visibility)
...
select test.allowed_for(test.subject_of('Coverage A'),
  format($q$insert into chat.message (thread_id, author_type, author_id, on_behalf_mode,
                                      body, visibility)
...
select test.eq(test.count_as(test.subject_of('Owner A'),
  $q$with denied as (
...
select test.allowed_for('family:parent-a',
...
update chat.contact set can_message = false where name = 'Parent A';

select test.denied_for('family:parent-a',
...
select test.denied_for('family:nobody-at-all',
