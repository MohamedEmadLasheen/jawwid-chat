...
describe("A/B/C/D — a device token belongs to exactly one actor", () => {
  ...
  const tokenRow = () =>
    g.prisma.deviceToken.findUnique({ where: { token: TOKEN } });
  ...
  it("D. an actor from another organization cannot retire it either", async () => {
    ...
    await g.prisma.$executeRawUnsafe(
      `delete from chat.staff where id = '${otherStaff}'::uuid`,
    );
    await g.prisma.$executeRawUnsafe(
      `delete from chat.organization where id = '${otherOrg}'::uuid`,
    );
  });
  ...
  it("retiring twice is idempotent", async () => {
    await notifications.unregisterDevice(TOKEN, s.parentId);
    await expect(
      notifications.unregisterDevice(TOKEN, s.parentId),
    ).resolves.toBeUndefined();
    expect((await tokenRow())?.isActive).toBe(false);
  });
...
describe("E/F/G — an incoming call notifies the callee, and only the callee", () => {
  ...
  it("incoming_call ignores quiet hours, because the rule says it may", async () => {
    const rule = await g.prisma.notificationRule.findUnique({
    ...
    expect(scheduled.scheduledAt.getTime()).toBeLessThanOrEqual(
      Date.now() + 2000,
    );
  });
...
describe("H/I — a missed call notifies whoever did not answer", () => {
  ...
  it("H. a timed-out call schedules missed_call from the same call.ended event", async () => {
    ...
    await ageCall(
      callId,
      (await g.config.get("call.ring_timeout_seconds")) + 5,
    );
  ...
  it("an answered call produces no missed_call at all", async () => {
    ...
    expect(
      (await notificationsFor(s.parentId)).filter(
        (n) => n.ruleKey === "missed_call",
      ),
    ).toHaveLength(0);
  });
...
describe("L/M — devices", () => {
  ...
  it("re-registering an existing token reactivates it rather than duplicating it", async () => {
    ...
    const rows = await g.prisma.deviceToken.findMany({
      where: { token: "phone" },
    });
...
describe("N/O/P/Q — security", () => {
  ...
  it("N. a revoked relationship cannot create a new call notification", async () => {
    ...
    // Reassignment is an ingestion act, not a Chat-side edit:
    // chat.guard_learner_assignment_change() refuses the latter. withAssignmentGate
    // opens the same transaction-local gate ingestion opens, which is what makes
    // this a faithful simulation of the relationship being revoked in Core
    // rather than a test writing a fact the application may not author.
    await withAssignmentGate(
      g.prisma,
      `update chat.learner set teacher_id = '${s.unrelatedTeacherId}'
  ...
  it("O. recipients come from the call record, so a forged payload redirects nothing", async () => {
    ...
    await g.prisma.$transaction(async (tx) => {
      await g.outbox.enqueue(
        tx,
        "call.incoming" as never,
        {
          callId: (await g.prisma.call.findFirst({
  ...
  it("Q. the push payload carries no media secret, no token and no contact channel", async () => {
    ...
    expect(carried).not.toMatch(
      /roomName|livekit|apiSecret|accessToken|callToken/i,
    );
