...
| `recovery/` | [PHASE-4-REPORT.md](recovery/PHASE-4-REPORT.md) | **what Phase 4 actually built and what it deliberately did not** — realtime authorization, presence, typing, the crash-safe outbox lease, the persistent offline queue, attachment key binding and object verification, APNs/FCM, notification preferences, grouping and deep links; the seven defects found on the way; the verification record |
| `recovery/` | [PHASE-5-REPORT.md](recovery/PHASE-5-REPORT.md) | **what Phase 5 actually built and what it deliberately did not** — the server-authoritative call state machine, missed calls, class calls and their reminders, follow-up call recording with retention, authorized playback and audit, stories with server-resolved audiences, broadcast with a leased fan-out and honest delivery tracking; the six defects found on the way (including a permissive RLS policy that would have cancelled every read policy in the same migration); the LiveKit deployment boundary; the verification record |
| `recovery/` | [PHASE-5-CLOSURE-REPORT.md](recovery/PHASE-5-CLOSURE-REPORT.md) | **the Phase 5 closure pass** — the LiveKit server the repository had been assuming, the Flutter media layer that makes calls carry audio, LiveKit Egress so recordings are captured rather than described, the native microphone and background-audio configuration, and the token contract verified against a real LiveKit; states plainly what still needs deployment, why CallKit is not implemented, and that recording retention is still a business decision |
| `recovery/` | [PHASE-5-ACCEPTANCE-REPORT.md](recovery/PHASE-5-ACCEPTANCE-REPORT.md) | **the Phase 5 acceptance audit** — ACCEPTED WITH EXTERNAL DEPENDENCIES; provenance proved by rebuilding Phase 5 from its own commits with a peer's four interleaved commits excluded; the feature matrix; the one defect found (a token-TTL fallback that could never run, and a report claim corrected); the RLS re-audit showing no permissive policy returned; and precisely what is implemented, verified, not verified, deployable and not deployed |
| `architecture/` | [PHASE-7-AI-ARCHITECTURE.md](architecture/PHASE-7-AI-ARCHITECTURE.md) | **the Phase 7 AI layer** — the reconnaissance record, the reuse contract, the provider seam, untrusted-content handling, and why the class/payment/renewal triggers are fed from the Core boundary rather than a local mirror |
| `recovery/` | [PHASE-6-REPORT.md](recovery/PHASE-6-REPORT.md) | **what Phase 6 actually built and what it deliberately did not** — the content scanner that replaced "hold every teacher message", the rule catalogue as data, regex safety and fail-closed scanning, the moderation queue with approve / edit-then-send / reject / delete, escalation to a manager on a configured threshold, and the Manager Command Center on live data; the three defects found on the way (including a fail-OPEN in which the scanner could not read its own rules under RLS and reported every message safe); why the frozen CRM dashboard was replaced rather than repaired; and the verification record |
| `recovery/` | [PHASE-7-REPORT.md](recovery/PHASE-7-REPORT.md) | **what Phase 7 actually built and what it deliberately did not** — the AI provider seam, approved knowledge and the grounded FAQ, suggested replies that cannot send themselves, conversation summaries, risk detection and attention flags, and the automation engine; the three governance guarantees and how each is made structural; why class/payment/renewal reminders are dormant rather than mirrored; the verification record |
| `recovery/` | [PHASE-8-REPORT.md](recovery/PHASE-8-REPORT.md) | **the production readiness report** — the readiness matrix, what Phase 8 found and fixed (a backup that restored into a database the application could not write to; a CI pipeline red at its first job; nothing bounding an authenticated session), and the honest verdict: the application is a Production Candidate, the delivery layer has never run |
| `recovery/` | [PHASE-8-CLOSURE.md](recovery/PHASE-8-CLOSURE.md) | **the authoritative readiness verdict** — the evidence matrix (Implemented / Local / CI / Staging / Production, used strictly), the blocker register, the mobile and load-test plans, the deployment checklist, and the five defects the closure pass found that were on no list |
| `recovery/` | [PHASE-8-EXTERNAL-GATE.md](recovery/PHASE-8-EXTERNAL-GATE.md) | **the final readiness verdict** — what is production-ready, what is only locally verified, what has never executed; the external blocker register with exit criteria, the owner's ordered next actions, and the three live defects this gate found (including a restore that passed every check while the application could not open the database) |
| `recovery/` | [PHASE-8-DELIVERY-HANDOFF.md](recovery/PHASE-8-DELIVERY-HANDOFF.md) | **the engineering→delivery handoff** — who does what next: the frozen baseline (tree-hash verified), every remaining item classified as repository work or external dependency, the B-4a metrics decision and its sequencing, what staging actually requires (and what it does not), and the owner/Claude action split |
| `qa/` | [performance-baseline.md](qa/performance-baseline.md) | **the first performance measurement this system has ever had** — service-layer latency and throughput, per-operation database work, and an explicit list of what was NOT measured |
| `recovery/` | [PHASE-8-FREEZE.md](recovery/PHASE-8-FREEZE.md) | **the Phase 8 freeze** — which items are COMPLETE, which are EXTERNAL BLOCKERS and which are DEFERRED BY DESIGN, stated so that a blocker is not read as an unfinished task; the gates verified at the freeze commit; and what must not be built next |
