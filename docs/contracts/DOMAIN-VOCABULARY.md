...
| **Authorized teacher–parent relationship** | The sole basis on which a teacher and a parent may hold a direct 1:1 conversation or call (**PD-6**). Holds when an active, messaging-capable Contact and an active Teacher are linked by a Learner — same family, that teacher assigned — inside one Organization. Derived from Jawwid Core data; **never** asserted by a client. Re-evaluated on every send, call start and media-token issue. | `chat.teacher_parent_authorized(uuid, uuid)` over `chat.contact` → `chat.learner` → `chat.teacher` | — (query) | not a wire field; a denial surfaces as `COMM.TEACHER_PARENT_NOT_AUTHORIZED` | — (the call/message affordance is shown only where the backend authorizes it) | — | canonical |
...
| **Participant** | A member row of a conversation: `actor_kind` + `actor_id` + `member_role` (`parent` · `teacher` · `admin` · `observer`) + `is_silent`. BR-1 — as re-versioned by PD-6 — is enforced against this set. **Wire/DB name is `member`; canonical prose is "participant".** | `chat.conversation_member` | `ConversationMember` | `members[]`, `member_role` (`MemberRole`), `MemberSpec` (`conversation.service.ts:23`) | `ParticipantRole` (display only) | (none yet) | canonical |
...
| **Call Participant** | An actor in a Call (`actor_kind` + `actor_id`, join/leave times). BR-1 — as re-versioned by PD-6, i.e. the authorized-relationship rule — is enforced here too (`20260905093000:679`). | `chat.call_participant` | `CallParticipant` | `participants[]` | — | — | canonical |
