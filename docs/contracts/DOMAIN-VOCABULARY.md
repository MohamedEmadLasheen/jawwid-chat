...
| **Group** | A durable BUSINESS entity: which students are taught together, by which teachers, with history. **NOT a conversation** — Phase 3 creates none and `chat.group` carries no `conversation_id`. The id is stable across rename, teacher change, membership change, closure and archival; a replacement group gets a NEW id. | `chat.group`, `chat.group_member`, `chat.group_teacher` (`20260907130100`) | `Group`, `GroupMember`, `GroupTeacher` | `group_id`, `state` (`active`\|`closed`\|`archived`) | — | `Group` (`shared/types/directory.ts`) | canonical (Phase 3) |
| **Teacher Assignment** | Learner → its teacher, with history. At most one live row per learner; `chat.learner.teacher_id` is a trigger-maintained mirror of it. Changing it is a **teacher transfer**, done only through `chat.assign_learner_teacher()`, with a reason. | `chat.learner_teacher_assignment` (`20260907130000`) | `LearnerTeacherAssignment` | `assignments[]`, `isCurrent` | — | `TeacherAssignment` | canonical (Phase 3) |
| **Label** | Reusable organizational metadata attached to families (VIP, Renewal, Wednesday). Many-to-many. **Nothing in authorization, scope or messaging reads a label.** Deletion is soft. | `chat.label`, `chat.family_label` (`20260907130200`) | `Label`, `FamilyLabel` | `label`, `labels[]` | — | `Label` | canonical (Phase 3) |
